---
name: bedlam-create-plugin
description: >
  Create new Bedlam plugins with the current alpha SDK/runtime. Use when
...
# Create a Bedlam Plugin

Use this skill when the task is to create, scaffold, or document a Bedlam plugin.
...
node packages/plugins/create-bedlam-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Bedlam repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.bedlam-sdk/`:

```bash
pnpm --filter @bedlam/create-bedlam-plugin build
node packages/plugins/create-bedlam-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Bedlam from an absolute local path during development
