...
This spec defines how Bedlam actually runs agents while staying runtime-agnostic.
...
1. Bedlam is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Bedlam manages lifecycle, and on exit Bedlam parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `bedlam.skill` (shared Bedlam skill text block)
...
2. Preferred transport is env vars (`BEDLAM_*`) injected at runtime.
