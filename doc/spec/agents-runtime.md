...
For `claude_local` and `codex_local`, Bedlam assumes the CLI is already installed and authenticated on the host machine.
...
Bedlam stores resumable session state per `(agent, taskKey, adapterType)`.
