...
ClipHub is the public registry where people share, discover, and download Bedlam company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Bedlam what a package registry is to a programming language. Bedlam already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Bedlam — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Bedlam instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Bedlam spec. It contains:
...
**Install (fresh start):**
```
bedlam install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Bedlam instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Bedlam, export your company as a template, then publish:

```
bedlam export --template my-company
bedlam publish cliphub my-company
```
...
ClipHub is a **separate service** from Bedlam itself. Bedlam is self-hosted; ClipHub is a hosted registry that Bedlam instances talk to.
...
| **Bedlam CLI** | `bedlam install`, `bedlam publish`, `bedlam cliphub sync` — built into Bedlam |
| **Bedlam UI** | "Browse ClipHub" panel in the Bedlam web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Bedlam) |
| Backend | TypeScript + Hono (consistent with Bedlam) |
| Database | PostgreSQL |
...
4. Run `bedlam install cliphub:acme/lean-saas-shop`
5. Bedlam creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Bedlam until it works well
2. Export: `bedlam export --template my-agency`
3. Publish: `bedlam publish cliphub my-agency`
...
3. Install just that agent: `bedlam install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Bedlam

ClipHub is **not required** to use Bedlam. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Bedlam what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `bedlam install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Bedlam web UI
- [ ] `bedlam cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Bedlam's own philosophy)
