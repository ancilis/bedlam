...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Bedlam plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Bedlam.
...
- package: `@bedlam/plugin-kitchen-sink-example`
- plugin id: `bedlam.kitchen-sink-example` or `bedlam-kitchen-sink-example`

Recommendation: use `bedlam-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Bedlam event such as `issue.created`
...
Recommendation: default to a Bedlam-local endpoint or a stable public echo endpoint to avoid flaky docs.
