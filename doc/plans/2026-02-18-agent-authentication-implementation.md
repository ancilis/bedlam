...
- New config/env settings:
  - `BEDLAM_AGENT_JWT_SECRET`
  - `BEDLAM_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `BEDLAM_AGENT_JWT_ISSUER` (default: `bedlam`)
  - `BEDLAM_AGENT_JWT_AUDIENCE` (default: `bedlam-api`)
...
1. In:
   ...
   inject `BEDLAM_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `BEDLAM_API_KEY`, do not overwrite it.
- Continue injecting:
  - `BEDLAM_AGENT_ID`
  - `BEDLAM_COMPANY_ID`
  - `BEDLAM_API_URL`
...
- Local adapters authenticate without manual `BEDLAM_API_KEY` config.
