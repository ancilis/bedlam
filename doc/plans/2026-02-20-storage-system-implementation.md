...
Add a single storage subsystem for Bedlam that supports:
...
- API serves attachment bytes through authenticated Bedlam endpoints.
...
- Default local path is under instance root: `~/.bedlam/instances/<instanceId>/data/storage`.
...
- `bedlam onboard` writes a valid `storage` config block by default.
- `bedlam configure --section storage` can switch between local and s3 modes.
...
- `bedlam doctor` reports actionable storage status.
