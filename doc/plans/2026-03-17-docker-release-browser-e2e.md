...
Today release smoke testing for published Bedlam packages is manual and shell-driven:
...
- runs `bedlam@${BEDLAMAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@bedlam.local`
- password: `bedlam-smoke-password`
...
- the suite passes locally against both:
  - `BEDLAMAI_VERSION=canary`
  - `BEDLAMAI_VERSION=latest`
...
1. A published `bedlam@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `bedlam@latest` Docker install can be smoke-tested by Playwright in CI.
