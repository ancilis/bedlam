...
`PAP-447` asks how Bedlam should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Bedlam instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Bedlam:

- some operators use Bedlam to manage Bedlam and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Bedlam
- Claude and Codex expose different built-in affordances, so Bedlam should not overfit to one tool

## Core Product Decision

Bedlam should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Bedlam
...
- Bedlam should not assume that every adapter can see or use a host filesystem path directly
...
If Bedlam makes worktrees agent-first, it will blur:
...
- Bedlam core resolves and records execution workspace state
...
That gives Bedlam room to support:
...
- Bedlam handles workspace runtime services under the hood using project/workspace policy
...
Once Bedlam is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Bedlam cannot infer
...
- when issue moves to done, should Bedlam auto-commit?
- should it auto-push?
...
Bedlam already has the right foundation for a project-first model.
...
For server-spawned local adapters, Bedlam already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Bedlam should mirror directly

Implication:

- for `codex_local`, Bedlam should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Bedlam should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Bedlam should separate:
...
Bedlam should be able to express intentions such as:
...
These are not specific to Bedlam. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".bedlam/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Bedlam-specific `devServer` flag.
...
Bedlam should distinguish between:
...
Bedlam should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Bedlam should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Bedlam
...
For runtime services specifically, Bedlam should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Bedlam-only dev-server flags
...
3. Inject resolved execution workspace details into `context.bedlamWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Bedlam instance can be reused through env/config injection when appropriate
...
- Bedlam can decide whether to reuse or start a fresh service deterministically
...
3. Test against Bedlam-on-Bedlam workflows first.
...
7. Workspace runtime services are modeled generically, not as Bedlam-specific dev-server toggles.
