...
This document defines the next-stage plan for Bedlam company import/export.

The core shift is:

- move from a Bedlam-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Bedlam.
...
Bedlam already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Bedlam
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `bedlam.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.bedlam.yaml` sidecar for high-fidelity Bedlam-specific details
...
11. adapter-aware skill sync surfaces so Bedlam can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Bedlam at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Bedlam sidecar.
...
Bedlam must not redefine `SKILL.md`.
...
- Bedlam-specific fidelity lives in `.bedlam.yaml`
- Bedlam may resolve and install `SKILL.md` packages, but it must not require a Bedlam-only skill format
...
### 5.4 Base Package Vs Bedlam Extension
...
- base package:
  ...
  - no Bedlam-only runtime fields by default
- Bedlam extension:
  - `.bedlam.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Bedlam tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`bedlam.manifest.json` is not part of the future package direction.
...
Bedlam import/export should support these entity kinds:
...
- Bedlam can treat “this set of agents came from team package X” as the imported-team model
...
Bedlam should support source references in package metadata with:
...
Bedlam should define a new adapter surface area around skills:
...
Planned Bedlam behavior:

- if an adapter supports read, Bedlam should show current skills in the UI
- if an adapter supports write, Bedlam should let the user enable/disable imported skills
- if an adapter supports sync, Bedlam should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.bedlam.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.bedlam.yaml` alongside the base package
...
## 10. Storage Model Inside Bedlam
...
Bedlam should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `bedlam company export <company-id> --out <path>`
- `bedlam company import <path-or-url> --dry-run`
- `bedlam company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Bedlam storage, or only referenced at import time?
...
3. Should Bedlam support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Bedlam aligned with:
