...
Bedlam needs to support two very different but equally valid ways of working:
...
Today, Bedlam already has the beginnings of this model:
...
- making Bedlam feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Bedlam users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Bedlam deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Bedlam into a full code review product
...
This object must be recorded explicitly so that Bedlam can:
...
Bedlam should treat PRs as a type of work product linked back to:
...
Bedlam already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Bedlam must be a first-class deployment mode

Bedlam cannot assume that it is running on the same machine as the code.

In cloud deployments, Bedlam may:
...
- `Runtime service`: a process or service Bedlam owns or tracks for a workspace
...
It also lets cloud-hosted Bedlam deployments point at codebases and remotes without pretending the Bedlam host has direct filesystem access.
...
Without an explicit `execution workspace` record, Bedlam has nowhere stable to attach:
...
Bedlam needs a single place to show:
...
- Bedlam-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Bedlam owning the execution host
...
## 1. Bedlam host and execution host must be decoupled
...
without the Bedlam host itself running local git or local preview processes.
...
Bedlam should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Bedlam.
...
Bedlam should keep workspace automation centered on a small set of hooks:
...
Bedlam should provide the equivalent visibility in the project `Code` surface:
...
Bedlam should treat execution workspaces as the natural home for:
...
Bedlam should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Bedlam should continue treating cleanup policy as part of the core workspace model:
...
Bedlam should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Bedlam host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Bedlam should borrow Worktrunk's good execution patterns while keeping the broader Bedlam model:
...
That keeps Bedlam focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Bedlam deployments remain first-class

That is a better fit for Bedlam than either extreme:
