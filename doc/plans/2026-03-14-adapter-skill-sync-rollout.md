...
This document defines the rollout plan for adapter-wide skill support in Bedlam.
...
- Bedlam stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Bedlam currently has these adapters:
...
1. Desired skills live in Bedlam for every adapter.
...
These adapters have a stable local skills directory that Bedlam can read and manage.
...
These adapters do not have a meaningful Bedlam-owned persistent install state.
...
- desired skills stored in Bedlam
...
- runtime already injects Bedlam skills into `~/.cursor/skills`
...
- runtime already injects Bedlam skills into `~/.gemini/skills`
...
- runtime already injects Bedlam skills into `~/.pi/agent/skills`
...
- Bedlam can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Bedlam skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Bedlam may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Bedlam skills that are clearly marked as Bedlam-managed
...
- Bedlam stores desired skills only
...
- avoid destructive wording unless Bedlam can prove a skill is Bedlam-managed
...
That gets Bedlam from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
