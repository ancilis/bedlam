...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/bedlam-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `BEDLAM_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `BEDLAM_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Bedlam may create host-local worktrees, processes, and paths
- `cloud`
  - Bedlam should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `bedlam | github | vercel | s3 | custom`
...
- use `BEDLAM_EXECUTION_TOPOLOGY` as a defaulting guardrail
