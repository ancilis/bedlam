...
Bedlam's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Bedlam's API, UI, and data model | Yes |
...
modules/
  observability/
    bedlam.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@bedlam/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@bedlam/core";
...
```
1. Scan modules/ directory for bedlam.module.json manifests
...
```jsonc
// bedlam.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@bedlam/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@bedlam/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:bedlam/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.bedlam.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm bedlam store list                    # browse available modules and templates
pnpm bedlam store install <module-id>     # install a module
pnpm bedlam store import <template-id>    # import a company template
pnpm bedlam store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Bedlam instance | `server:started` |
...
4. **Module config** — `bedlam.config.json` with per-module config, env var interpolation
...
10. **`@bedlam/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
