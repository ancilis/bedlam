...
This document defines the V1 design for a Bedlam adapter that integrates with
...
4. synthesized stdout events for Bedlam UI/CLI
...
3. **Agent callback auth to Bedlam** uses a bootstrap exchange flow (no long-lived Bedlam key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Bedlam endpoints, not full SKILL.md prompt injection.
...
- `bedlamPublicUrl` (optional override; else `BEDLAM_PUBLIC_URL` env)
...
## Bedlam Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `BEDLAM_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.bedlamPublicUrl`
2. `process.env.BEDLAM_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Bedlam credentials in prompt text.

Flow:

1. Before launch/follow-up, Bedlam mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Bedlam validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Bedlam API calls.
...
1. Prompt includes a compact instruction to fetch skills from Bedlam.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/bedlam`
   - `GET /api/skills/bedlam-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <bedlamPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Bedlam run context.
...
5. `bedlamPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Bedlam skill endpoints |
| Bedlam auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Bedlam context.
3. Add richer PR metadata surfacing in Bedlam UI.
