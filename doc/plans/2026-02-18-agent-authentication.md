...
Agents need API keys to authenticate with Bedlam. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Bedlam
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Bedlam generates a token and passes it directly to the agent
...
`BEDLAM_API_URL`, `BEDLAM_AGENT_ID`, `BEDLAM_COMPANY_ID`. We need to
add a `BEDLAM_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Bedlam CLI
...
```
bedlam auth login
# Opens browser -> user confirms -> token stored at ~/.bedlam/credentials
```
...
managed by the Bedlam server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Bedlam.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Bedlam SKILL.md (or a link to it)
   - What information Bedlam needs from the agent (e.g. webhook URL, adapter
...
5. Bedlam stores the pending registration.
...
7. On approval, Bedlam provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Bedlam issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.bedlam.ing/skills/bedlam/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Bedlam should send you heartbeats",
...
1. Generate a Bedlam invite link for the company.
...
4. A Bedlam company member approves the new agent.
5. Bedlam begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `BEDLAM_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `bedlam auth login` for developer-managed remote agents.                                      |
