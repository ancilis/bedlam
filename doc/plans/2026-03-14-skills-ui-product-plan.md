...
This document defines the product and UI plan for skill management in Bedlam.
...
- Bedlam company import/export can include skills as package content
...
3. The UI must always tell the truth about what Bedlam knows:
   - desired state in Bedlam
...
Bedlam should treat skills at two scopes:
...
- created locally inside Bedlam later
...
7. “Understand what Bedlam knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Bedlam-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Bedlam did not install.

These should be clearly marked:

- external
- not managed by Bedlam
...
- `desired_only`: Bedlam wants it, adapter does not show it yet
- `external`: adapter has it but Bedlam does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Bedlam only tracks desired state
...
- Bedlam can still track desired skills
...
- `.bedlam.yaml` may add Bedlam-specific fidelity, but should not replace the base shortname association model
...
- `.bedlam.yaml` may add Bedlam-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Bedlam should have a company-scoped skill model or managed package model representing:
...
Bedlam should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Bedlam-specific.
...
That gives Bedlam one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
