# Bedlam Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Bedlam's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Bedlam app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Bedlam HTTP APIs directly.
...
Bedlam plugin design is based on the following assumptions:

1. Bedlam is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Bedlam business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Bedlam core.
...
2. Let plugins add major capabilities without editing Bedlam core.
...
4. Depend on project-local plugin folders such as `.bedlam/plugins`.
...
The single Bedlam deployment an operator installs and controls.

### 5.2 Company

A first-class Bedlam business object inside the instance.
...
A trusted in-process extension loaded directly by Bedlam core.
...
An installable instance-wide extension package loaded through the Bedlam plugin runtime.
...
Bedlam has two extension classes.
...
Bedlam already has a concrete workspace model:
...
Plugins live under the Bedlam instance directory.

Suggested layout:

- `~/.bedlam/instances/default/plugins/package.json`
- `~/.bedlam/instances/default/plugins/node_modules/`
- `~/.bedlam/instances/default/plugins/.cache/`
- `~/.bedlam/instances/default/data/plugins/<plugin-id>/`
...
Bedlam should add CLI commands:

- `pnpm bedlam plugin list`
- `pnpm bedlam plugin install <package[@version]>`
- `pnpm bedlam plugin uninstall <plugin-id>`
- `pnpm bedlam plugin upgrade <plugin-id> [version]`
- `pnpm bedlam plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@bedlam/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface BedlamPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Bedlam agents can use during runs.
...
- Bedlam server starts one worker process per installed plugin
...
Receives one typed Bedlam domain event.
...
export function definePlugin(definition: PluginDefinition): BedlamPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: BedlamPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@bedlam/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@bedlam/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@bedlam/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@bedlam/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@bedlam/plugin-sdk/ui`
...
The `@bedlam/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Bedlam secret provider system rather than a plain text input
...
1. Core Bedlam data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Bedlam objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Bedlam product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Bedlam later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Bedlam secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm bedlam plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Bedlam server. This is a normative requirement, not optional.
...
### 27.1 `@bedlam/plugin-test-harness`
...
```ts
import { createTestHarness } from "@bedlam/plugin-test-harness";
...
For developing a plugin against a running Bedlam instance:

- The operator installs the plugin from a local path: `pnpm bedlam plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-bedlam-plugin`) that scaffolds:

- `package.json` with correct `bedlamPlugin` keys
...
- `@bedlam/plugin-workspace-files`
- `@bedlam/plugin-terminal`
- `@bedlam/plugin-git`
- `@bedlam/plugin-linear`
- `@bedlam/plugin-github-issues`
- `@bedlam/plugin-grafana`
- `@bedlam/plugin-runtime-processes`
- `@bedlam/plugin-stripe`
...
- `@bedlam/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@bedlam/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@bedlam/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@bedlam/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@bedlam/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@bedlam/plugin-sdk/ui`
...
- `@bedlam/plugin-test-harness`
- `create-bedlam-plugin` starter template
...
Bedlam should not implement a generic in-process hook bag modeled directly after local coding tools.

Bedlam should implement:
...
That is the complete target design for the Bedlam plugin system.
