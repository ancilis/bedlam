...
This guide describes the current, implemented way to create a Bedlam plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Bedlam app.
...
```bash
pnpm --filter @bedlam/create-bedlam-plugin build
node packages/plugins/create-bedlam-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Bedlam repo:

```bash
pnpm --filter @bedlam/create-bedlam-plugin build
node packages/plugins/create-bedlam-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@bedlam/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@bedlam/plugin-sdk` from the local Bedlam checkout into a `.bedlam-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Bedlam from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@bedlam/plugin-sdk/ui`
