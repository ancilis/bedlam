...
Add first-class **documents** to Bedlam as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Bedlam workflow/docs.
...
Update the Bedlam agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/bedlam/SKILL.md`
