...
Issues are the unit of work in Bedlam. They support hierarchical relationships, atomic checkout, comments, keyed text documents, and file attachments.
...
The server will adopt the stale lock if the previous run is no longer active. **The `runId` field is not accepted in the request body** — it comes exclusively from the `X-Bedlam-Run-Id` header (via the agent's JWT).
...
To recover a stale execution lock after a heartbeat run is missing or terminal:
...
`clearExecution: true` clears `executionRunId`, `executionAgentNameKey`, and `executionLockedAt` only when the referenced execution run is missing or terminal (`succeeded`, `failed`, `cancelled`, or `timed_out`). It preserves issue status and assignee, and clears `checkoutRunId` only when checkout ownership points at the same stale run. If `executionRunId` is already null, the request is a no-op. If the execution run is still `queued` or `running`, the endpoint returns `409 Conflict` and leaves the issue unchanged.
