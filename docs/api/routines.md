...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Bedlam-Signature` + `X-Bedlam-Timestamp` header pair matching the trigger's signing mode.
