# ClipHub: Marketplace for Bedlam Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Bedlam configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Bedlam-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Bedlam company (agents, projects, governance) |
...
A complete Bedlam company configuration:
...
Single-agent configurations designed to plug into a Bedlam org:
...
Portable skill files that any Bedlam agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Bedlam's skill loading system
...
interface Listing {
  ...
  bedlamVersionMin: string;     // Minimum Bedlam version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Bedlam company
...
| `POST` | `/api/listings/:id/install` | Install to Bedlam company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Bedlam company (or create new)
  → ClipHub API calls Bedlam API to:
  ...
  → Redirect to Bedlam dashboard with new team running
```
...
### 5.3 Creator: Export from Bedlam → Publish

```
Running Bedlam company → "Export as Blueprint" (CLI or UI)
  → Bedlam exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Bedlam brand blue/purple)
...
| Install flow | One-click deploy to Bedlam company |
...
{
  "targetCompanyId": "uuid",         // Existing Bedlam company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `bedlam-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Bedlam
- **Backend**: Node.js API (or extend Bedlam server)
- **Database**: Postgres (can share Bedlam's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Bedlam auth (or OAuth2)

### 9.2 Integration with Bedlam

ClipHub can be:
- **Option A**: A separate app that calls Bedlam's API to install blueprints
- **Option B**: A built-in section of the Bedlam UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Bedlam UI and API.
...
- [ ] Install flow: blueprint → Bedlam company
...
- [ ] "Export from Bedlam" CLI command
