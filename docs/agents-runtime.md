...
Audience: Operators setting up and running agents in Bedlam

## 1. What this system does

Agents in Bedlam do not run continuously.  
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`), Bedlam assumes the CLI is already installed and authenticated on the host machine.
...
Bedlam stores session IDs for resumable adapters.
...
Bedlam pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Bedlam surfaces this as a warning in environment tests, not a hard error.
