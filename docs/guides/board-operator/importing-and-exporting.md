...
Bedlam companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .bedlam.yaml     # Adapter config, env inputs, routines
```
...
- **.bedlam.yaml** holds Bedlam-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
bedlam company export <company-id> --out ./my-export
```
...
bedlam company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
bedlam company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
bedlam company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.bedlam.yaml`
...
```sh
# From a local folder
bedlam company import ./my-export

# From a GitHub URL
bedlam company import https://github.com/org/repo

# From a GitHub subfolder
bedlam company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
bedlam company import org/repo
bedlam company import org/repo/companies/acme
```
...
If `--target` is not specified, Bedlam infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
bedlam company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
bedlam company import org/company-templates/engineering-team \
  --target new \
...
```sh
bedlam company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
bedlam company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
bedlam company import ./package \
  --target new \
  --yes \
  --json
```
...
Bedlam supports several GitHub URL formats:
