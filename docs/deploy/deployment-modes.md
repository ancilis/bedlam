...
Bedlam supports two runtime modes with different security profiles.
...
```sh
pnpm bedlam allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Bedlam emits a one-time claim URL at startup:
...
```sh
BEDLAM_DEPLOYMENT_MODE=authenticated pnpm bedlam run
```
