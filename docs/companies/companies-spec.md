...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Bedlam.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.bedlam.yaml`
...
- Bedlam must not require extra top-level fields for skill validity
- Bedlam-specific extensions must live under `metadata.bedlam` or `metadata.sources`
...
For Bedlam, the preferred fidelity extension is:
...
- issue/task Bedlam-only metadata
...
- Bedlam tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Bedlam shape:
...
Additional rules for Bedlam exporters:
...
- omit empty and default-valued Bedlam fields when possible
...
- Bedlam exporters should emit `.bedlam.yaml` as a sidecar by default
...
Bedlam can map this spec to its runtime model like this:
...
- Bedlam extension:
  - `.bedlam.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Bedlam-specific fidelity

Inline Bedlam-only metadata that must live inside a shared markdown file should use:

- `metadata.bedlam`

That keeps the base format broader than Bedlam.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Bedlam.

## 20. Cutover

Bedlam should cut over to this markdown-first package model as the primary portability format.

`bedlam.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Bedlam, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
