...
Build a custom adapter to connect Bedlam to any agent runtime.
...
2. Build environment with `buildBedlamEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `bedlam run --watch` using `picocolors`.
...
Make Bedlam skills discoverable to your agent runtime without writing to the agent's working directory:
