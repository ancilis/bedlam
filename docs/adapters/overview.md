...
summary: What adapters are and how they connect agents to Bedlam
---

Adapters are the bridge between Bedlam's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, Bedlam:
...
| [Hermes Local](/adapters/hermes-local) | `hermes_local` | Runs Hermes CLI locally |
| Cursor | `cursor` | Runs Cursor in background mode |
| Pi Local | `pi_local` | Runs an embedded Pi agent locally |
| OpenClaw Gateway | `openclaw_gateway` | Connects to an OpenClaw gateway endpoint (upstream Paperclip ecosystem adapter) |
| [OpenRouter](/adapters/openrouter) | `openrouter` | Calls any model via OpenRouter API — Anthropic, OpenAI, Google, Meta, and more |
...
packages/adapters/<name>/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `bedlam run --watch`
```
