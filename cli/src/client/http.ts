...
export class BedlamApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-bedlam-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Bedlam API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Bedlam server is not running, the configured URL is wrong, or the request is being blocked before it reaches Bedlam.",
    "",
    "Try:",
    "- Start Bedlam with `pnpm dev` or `pnpm bedlam run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Bedlam is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`BEDLAM_API_URL\`.`,
  );
  return lines.join("\n");
}
