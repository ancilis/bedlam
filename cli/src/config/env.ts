...
const JWT_SECRET_ENV_KEY = "BEDLAM_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Bedlam CLI commands",
...
export function resolveBedlamEnvFile(configPath?: string): string {
...
export function loadBedlamEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeBedlamEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readBedlamEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeBedlamEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeBedlamEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readBedlamEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeBedlamEnvEntries(next, filePath);
  return next;
}
