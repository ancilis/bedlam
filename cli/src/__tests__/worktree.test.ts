...
function buildSourceConfig(): BedlamConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("bedlam-pr-432")).toBe(
      path.resolve(os.homedir(), "bedlam-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("bedlam/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://bedlam.example", 3110)).toBe("https://bedlam.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/bedlam-feature",
      homeDir: "/tmp/bedlam-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/bedlam-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/bedlam-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.BEDLAM_HOME).toBe(path.resolve("/tmp/bedlam-worktrees"));
    expect(env.BEDLAM_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.BEDLAM_IN_WORKTREE).toBe("true");
    expect(env.BEDLAM_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.BEDLAM_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export BEDLAM_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bedlam-worktree-secrets-"));
    const originalInlineMasterKey = process.env.BEDLAM_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.BEDLAM_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.BEDLAM_SECRETS_MASTER_KEY;
      delete process.env.BEDLAM_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.BEDLAM_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.BEDLAM_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.BEDLAM_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          BEDLAM_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bedlam-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.BEDLAM_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.BEDLAM_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".bedlam-worktrees"),
      });

      const envPath = path.join(repoRoot, ".bedlam", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("BEDLAM_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("BEDLAM_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/BEDLAM_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.BEDLAM_AGENT_JWT_SECRET;
      } else {
        process.env.BEDLAM_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bedlam-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".bedlam-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".bedlam", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Bedlam config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bedlam-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".bedlam", "config.json");
    const originalCwd = process.cwd();
    const originalBedlamConfig = process.env.BEDLAM_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalBedlamConfig === undefined) {
        delete process.env.BEDLAM_CONFIG;
      } else {
        process.env.BEDLAM_CONFIG = originalBedlamConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bedlam-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/bedlam",
        targetRepoRoot: "/Users/example/bedlam-pr-432",
        workspaceCwd: "/Users/example/bedlam",
      }),
    ).toBe("/Users/example/bedlam-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/bedlam",
        targetRepoRoot: "/Users/example/bedlam-pr-432",
        workspaceCwd: "/Users/example/bedlam/packages/db",
      }),
    ).toBe("/Users/example/bedlam-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bedlam-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bedlam-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "bedlam-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("bedlam-make-test", {
        seed: false,
        home: path.join(tempRoot, ".bedlam-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".bedlam", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".bedlam", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
