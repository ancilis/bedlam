...
import { ApiConnectionError, ApiRequestError, BedlamApiClient } from "../client/http.js";

describe("BedlamApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new BedlamApiClient({
    ...
    expect(headers["x-bedlam-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new BedlamApiClient({ apiBase: "http://localhost:3100" });
