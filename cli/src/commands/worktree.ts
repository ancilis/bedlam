...
import { ensureAgentJwtSecret, loadBedlamEnvFile, mergeBedlamEnvEntries, readBedlamEnvEntries, resolveBedlamEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { BedlamConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printBedlamCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.BEDLAM_CONFIG;
...
const WORKTREE_NAME_PREFIX = "bedlam-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.BEDLAM_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.BEDLAM_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromBedlamConfig(config: BedlamConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromBedlamConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.bedlam${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".bedlam", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".bedlam", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.bedlam"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: BedlamConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://bedlam:bedlam@127.0.0.1:${port}/bedlam`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: BedlamConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.BEDLAM_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.BEDLAM_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.BEDLAM_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.BEDLAM_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: BedlamConfig;
  targetConfig: BedlamConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveBedlamEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readBedlamEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://bedlam:bedlam@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "bedlam");
    const targetConnectionString = `postgres://bedlam:bedlam@127.0.0.1:${targetHandle.port}/bedlam`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readBedlamEnvEntries(resolveBedlamEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.BEDLAM_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.BEDLAM_AGENT_JWT_SECRET);
  mergeBedlamEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { BEDLAM_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadBedlamEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Bedlam inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printBedlamCliBanner();
  p.intro(pc.bgCyan(pc.black(" bedlam worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printBedlamCliBanner();
  p.intro(pc.bgCyan(pc.black(" bedlam worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasBedlamConfig: existsSync(path.resolve(worktreePath, ".bedlam", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printBedlamCliBanner();
  p.intro(pc.bgCyan(pc.black(" bedlam worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveBedlamEnvFile(configPath);
  const envEntries = readBedlamEnvEntries(envPath);
  const out = {
    BEDLAM_CONFIG: configPath,
    ...(envEntries.BEDLAM_HOME ? { BEDLAM_HOME: envEntries.BEDLAM_HOME } : {}),
    ...(envEntries.BEDLAM_INSTANCE_ID ? { BEDLAM_INSTANCE_ID: envEntries.BEDLAM_INSTANCE_ID } : {}),
    ...(envEntries.BEDLAM_CONTEXT ? { BEDLAM_CONTEXT: envEntries.BEDLAM_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasBedlamConfig)
      .map((choice) => path.resolve(choice.worktree, ".bedlam", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readBedlamEnvEntries(resolveBedlamEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Bedlam once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasBedlamConfig ? "bedlam" : "no-bedlam-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".bedlam", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".bedlam", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .bedlam/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasBedlamConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Bedlam worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasBedlamConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Bedlam worktrees were found. Run `bedlam worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Bedlam configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Bedlam instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Bedlam instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with bedlam- if needed (created at ~/bedlam-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: BEDLAM_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: BEDLAM_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source BEDLAM_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Bedlam instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Bedlam worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with bedlam- if needed")
