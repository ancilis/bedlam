...
type OnboardDefaults = Pick<BedlamConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "BEDLAM_PUBLIC_URL",
  "DATABASE_URL",
  "BEDLAM_DB_BACKUP_ENABLED",
  "BEDLAM_DB_BACKUP_INTERVAL_MINUTES",
  "BEDLAM_DB_BACKUP_RETENTION_DAYS",
  "BEDLAM_DB_BACKUP_DIR",
  "BEDLAM_DEPLOYMENT_MODE",
  "BEDLAM_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "BEDLAM_ALLOWED_HOSTNAMES",
  "BEDLAM_AUTH_BASE_URL_MODE",
  "BEDLAM_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "BEDLAM_STORAGE_PROVIDER",
  "BEDLAM_STORAGE_LOCAL_DIR",
  "BEDLAM_STORAGE_S3_BUCKET",
  "BEDLAM_STORAGE_S3_REGION",
  "BEDLAM_STORAGE_S3_ENDPOINT",
  "BEDLAM_STORAGE_S3_PREFIX",
  "BEDLAM_STORAGE_S3_FORCE_PATH_STYLE",
  "BEDLAM_SECRETS_PROVIDER",
  "BEDLAM_SECRETS_STRICT_MODE",
  "BEDLAM_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveBedlamInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.BEDLAM_PUBLIC_URL?.trim() ||
    process.env.BEDLAM_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.BEDLAM_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.BEDLAM_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.BEDLAM_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.BEDLAM_ALLOWED_HOSTNAMES
    ? process.env.BEDLAM_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.BEDLAM_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.BEDLAM_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.BEDLAM_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.BEDLAM_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.BEDLAM_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.BEDLAM_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.BEDLAM_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.BEDLAM_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.BEDLAM_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.BEDLAM_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.BEDLAM_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.BEDLAM_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.BEDLAM_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.BEDLAM_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.BEDLAM_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "BEDLAM_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<BedlamConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printBedlamCliBanner();
  p.intro(pc.bgCyan(pc.black(" bedlam onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveBedlamInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: BedlamConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Bedlam install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("bedlam configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("BEDLAM_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.BEDLAM_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("BEDLAM_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("BEDLAM_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: BEDLAM_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("bedlam run")}`,
        `Reconfigure later: ${pc.cyan("bedlam configure")}`,
        `Diagnose setup: ${pc.cyan("bedlam doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.BEDLAM_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Bedlam setup is ready.");
    return;
  }
  ...
  let llm: BedlamConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@bedlam/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `bedlam doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("bedlam run")}`,
        `Then: ${pc.cyan("bedlam auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
