...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@bedlam/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://bedlam:bedlam@127.0.0.1:${port}/bedlam`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" bedlam db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveBedlamInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "bedlam";
