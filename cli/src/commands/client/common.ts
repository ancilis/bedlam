...
import { ApiRequestError, BedlamApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Bedlam config file")
    .option("-d, --data-dir <path>", "Bedlam data directory root (isolates state from ~/.bedlam)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Bedlam API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.BEDLAM_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.BEDLAM_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.BEDLAM_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set BEDLAM_COMPANY_ID, or set context profile companyId via `bedlam context set`.",
    );
  }

  const api = new BedlamApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.BEDLAM_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.BEDLAM_SERVER_PORT || "");
