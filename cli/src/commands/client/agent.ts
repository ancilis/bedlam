...
import type { Agent } from "@bedlam/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveBedlamSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export BEDLAM_API_URL='${escaped(input.apiBase)}'`,
    `export BEDLAM_COMPANY_ID='${escaped(input.companyId)}'`,
    `export BEDLAM_AGENT_ID='${escaped(input.agentId)}'`,
    `export BEDLAM_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Bedlam skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Bedlam skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveBedlamSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Bedlam skills directory. Expected ./skills in the repo checkout.",
              );
            }
