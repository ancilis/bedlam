...
export const DEFAULT_WORKTREE_HOME = "~/.bedlam-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".bedlam");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: BedlamConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "bedlam",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    BEDLAM_HOME: paths.homeDir,
    BEDLAM_INSTANCE_ID: paths.instanceId,
    BEDLAM_CONFIG: paths.configPath,
    BEDLAM_CONTEXT: paths.contextPath,
    BEDLAM_IN_WORKTREE: "true",
    ...(branding?.name ? { BEDLAM_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { BEDLAM_WORKTREE_COLOR: branding.color } : {}),
  };
}
