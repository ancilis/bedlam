/**
 * esbuild configuration for building the bedlam CLI for npm.
 *
 * Bundles all workspace packages (@bedlam/*) into a single file.
...
// to npm and resolved at runtime (e.g. @bedlam/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@bedlam/")) {
      externals.add(name);
    }
  }
