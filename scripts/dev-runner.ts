...
const devServerStatusFilePath = path.join(repoRoot, ".bedlam", "dev-server-status.json");
...
const env: NodeJS.ProcessEnv = {
  ...process.env,
  BEDLAM_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.BEDLAM_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.BEDLAM_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  env.BEDLAM_MIGRATION_PROMPT ??= "never";
  env.BEDLAM_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.BEDLAM_DEPLOYMENT_MODE = "authenticated";
  env.BEDLAM_DEPLOYMENT_EXPOSURE = "private";
  env.BEDLAM_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[bedlam] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[bedlam] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[bedlam] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@bedlam/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[bedlam] Command failed with code ${status.code}: pnpm --filter @bedlam/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[bedlam] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.BEDLAM_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[bedlam] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[bedlam] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@bedlam/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[bedlam] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@bedlam/server", serverScript, ...forwardedArgs],
