#!/usr/bin/env bash
#
# Kill all local Bedlam dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all bedlam dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/bedlam(-[^/]+)?/' | grep node | grep -v grep || true)

candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.bedlam/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.bedlam/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.bedlam/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/bedlam*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.bedlam/instances/*/db/postmaster.pid
    "$sibling_root"/.bedlam/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No Bedlam dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Bedlam dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Bedlam node processes..."
