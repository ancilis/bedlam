...
const DEFAULT_QUERY = "\"Co-Authored-By: Bedlam <noreply@bedlam.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/bedlam-commit-metrics-cache.json");
...
const BEDLAM_EMAIL = "noreply@bedlam.ing";
const BEDLAM_NAME = "bedlam";
...
function printHelp() {
  console.log(`Usage: tsx scripts/bedlam-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === BEDLAM_EMAIL) || (displayName && displayName.toLowerCase() === BEDLAM_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Bedlam commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "bedlam-commit-metrics",
