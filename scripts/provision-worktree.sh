...
base_cwd="${BEDLAM_WORKSPACE_BASE_CWD:?BEDLAM_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${BEDLAM_WORKSPACE_CWD:?BEDLAM_WORKSPACE_CWD is required}"
bedlam_home="${BEDLAM_HOME:-$HOME/.bedlam}"
bedlam_instance_id="${BEDLAM_INSTANCE_ID:-default}"
bedlam_dir="$worktree_cwd/.bedlam"
worktree_config_path="$bedlam_dir/config.json"
worktree_env_path="$bedlam_dir/.env"
worktree_name="${BEDLAM_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${BEDLAM_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.bedlam/config.json" || -L "$base_cwd/.bedlam/config.json" ) ]]; then
  source_config_path="$base_cwd/.bedlam/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$bedlam_home/instances/$bedlam_instance_id/config.json"
fi
...
run_isolated_worktree_init() {
  if command -v pnpm >/dev/null 2>&1 && pnpm bedlam --help >/dev/null 2>&1; then
    pnpm bedlam worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi

  if command -v bedlam >/dev/null 2>&1; then
    bedlam worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  BEDLAM_DIR="$bedlam_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  BEDLAM_WORKTREES_DIR="${BEDLAM_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const bedlamDir = process.env.BEDLAM_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.BEDLAM_WORKTREES_DIR) ?? "~/.bedlam-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(bedlamDir, "config.json");
  const envPath = path.resolve(bedlamDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "bedlam",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.BEDLAM_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.BEDLAM_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.BEDLAM_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "BEDLAM_HOME=" + JSON.stringify(worktreeHome),
    "BEDLAM_INSTANCE_ID=" + JSON.stringify(instanceId),
    "BEDLAM_CONFIG=" + JSON.stringify(configPath),
    "BEDLAM_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "BEDLAM_IN_WORKTREE=true",
    "BEDLAM_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.BEDLAM_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("BEDLAM_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "bedlam CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
