...
BEDLAM_HOST_PORT="${BEDLAM_HOST_PORT:-3100}"
BEDLAM_HOST_FROM_CONTAINER="${BEDLAM_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.bedlam-openclaw.override.yml"
...
detect_bedlam_base_url() {
  ...
  for candidate in "$BEDLAM_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${BEDLAM_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${BEDLAM_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
bedlam_base_url="$(detect_bedlam_base_url || true)"
...
Bedlam URL for OpenClaw container:
EOF
  if [[ -n "$bedlam_base_url" ]]; then
    cat <<EOF
  $bedlam_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${BEDLAM_HOST_PORT}
  (Do not use http://127.0.0.1:${BEDLAM_HOST_PORT} inside the container.)
  If Bedlam rejects the host, run on host machine:
    pnpm bedlam allowed-hostname host.docker.internal
  Then restart Bedlam and re-run this script.
EOF
  fi
  cat <<EOF
