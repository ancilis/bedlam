...
COMPANY_ID="${COMPANY_ID:-${BEDLAM_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-bedlam-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-bedlam-openclaw-smoke}"
...
Provide one of:
  BEDLAM_AUTH_HEADER=\"Bearer <board-token>\"
  BEDLAM_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Bedlam OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.bedlam.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
