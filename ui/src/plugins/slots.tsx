 ...
 *
 * Plugin bundles are built with `external: ["@bedlam/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__bedlamPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__bedlamPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__bedlamPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__bedlamPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@bedlam/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@bedlam/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@bedlam/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@bedlam/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@bedlam/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__bedlamPluginBridge__) {
 ...
 * self-register via `window.bedlamPlugins.registerReactComponent()`.
 ...
 * of `@bedlam/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
