...
/**
 * Filter .bedlam.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterBedlamYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://bedlam.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm bedlam company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Bedlam](https://bedlam.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Bedlam](https://bedlam.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .bedlam.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .bedlam.yaml
    const yamlPath = exportData.bedlamExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterBedlamYaml(exportData.files[yamlPath], checkedFiles);
    }
