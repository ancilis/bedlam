...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Bedlam API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "bedlamApiUrl",
                  String(config.bedlamApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "bedlamApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://bedlam.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "bedlam"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Bedlam persists a device key during onboarding so pairing approvals
