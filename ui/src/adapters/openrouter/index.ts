...
export const openRouterUIAdapter: UIAdapterModule = {
  ...
  parseStdoutLine: (line: string, ts: string) => {
    ...
    if (parsed.type === "output") {
      return [{ kind: "assistant", ts, text: parsed.text }];
    }
    if (parsed.type === "error") {
      return [{ kind: "stderr", ts, text: parsed.text }];
    }
    return [{ kind: "system", ts, text: parsed.text }];
  },
  ConfigFields: OpenRouterConfigFields,
  buildAdapterConfig: (values) => buildOpenRouterConfig(values as unknown as Record<string, unknown>),
};
