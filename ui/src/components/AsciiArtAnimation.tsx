...
type BedlamSprite = (typeof BEDLAM_SPRITES)[number];
...
function spriteSize(sprite: BedlamSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = BEDLAM_SPRITES[(baseCol + baseRow) % BEDLAM_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = BEDLAM_SPRITES[Math.floor(Math.random() * BEDLAM_SPRITES.length)]!;
