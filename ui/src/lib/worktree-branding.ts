...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("bedlam-worktree-enabled") !== "true") return null;

  const name = readMetaContent("bedlam-worktree-name");
  const color = normalizeHexColor(readMetaContent("bedlam-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("bedlam-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
