...
import { agents as agentsTable, companies, heartbeatRuns } from "@bedlam/db";
...
import { runClaudeLogin } from "@bedlam/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@bedlam/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@bedlam/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@bedlam/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@bedlam/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.BEDLAM_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeBedlamSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readBedlamSkillSyncPreference(
