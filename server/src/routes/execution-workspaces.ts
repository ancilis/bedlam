...
import { issues, projects, projectWorkspaces } from "@bedlam/db";
import { updateExecutionWorkspaceSchema } from "@bedlam/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  router.post("/execution-workspaces/:id/runtime-services/:action", async (req, res) => {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before Bedlam can manage local runtime services" });
      return;
    }
