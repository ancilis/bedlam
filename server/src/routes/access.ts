...
import type { DeploymentExposure, DeploymentMode } from "@bedlam/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "bedlam" &&
    normalized !== "bedlam-create-agent" &&
    normalized !== "bedlam-create-plugin" &&
...
/** Resolve the Bedlam repo skills directory (built-in / managed skills). */
function resolveBedlamSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const bedlamSkillsDir = resolveBedlamSkillsDir();

  // Build set of Bedlam-managed skill names
  const bedlamSkillNames = new Set<string>();
  if (bedlamSkillsDir) {
    try {
      for (const entry of fs.readdirSync(bedlamSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) bedlamSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isBedlamManaged: bedlamSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.bedlamApiUrl)) {
    const legacyBedlamApiUrl = nonEmptyTrimmedString(input.bedlamApiUrl);
    if (legacyBedlamApiUrl) merged.bedlamApiUrl = legacyBedlamApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    bedlamApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.bedlamApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawBedlamApiUrl =
    typeof defaults.bedlamApiUrl === "string"
      ? defaults.bedlamApiUrl.trim()
      : "";
  if (rawBedlamApiUrl) {
    try {
      const parsedBedlamApiUrl = new URL(rawBedlamApiUrl);
      if (
        parsedBedlamApiUrl.protocol !== "http:" &&
        parsedBedlamApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_bedlam_api_url_protocol",
          level: "warn",
          message: `bedlamApiUrl must use http:// or https:// (got ${parsedBedlamApiUrl.protocol}).`
        });
      } else {
        normalized.bedlamApiUrl = parsedBedlamApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_bedlam_api_url_configured",
          level: "info",
          message: `bedlamApiUrl set to ${parsedBedlamApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_bedlam_api_url_invalid",
        level: "warn",
        message: `Invalid bedlamApiUrl: ${rawBedlamApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Bedlam host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Bedlam is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm bedlam allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/bedlam";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/bedlam-claimed-api-key.json and load BEDLAM_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: bedlamApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Bedlam hostname is reachable and allowed via `pnpm bedlam allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Bedlam API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/bedlam/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Bedlam OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          bedlamApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Bedlam generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "bedlamApiUrl": "https://bedlam-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Bedlam before key claim is allowed.
    ...
    - ~/.openclaw/workspace/bedlam-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/bedlam-claimed-api-key.json

    And set the BEDLAM_API_KEY and BEDLAM_API_URL in your environment variables as specified here:
    ...
    Then set BEDLAM_API_KEY and BEDLAM_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Bedlam skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your BEDLAM_API_URL to the top of your skill and note the path to your BEDLAM_API_URL
    ...
    ${
      ...
      "Ensure Bedlam is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Bedlam base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.bedlamApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm bedlam allowed-hostname <host>
      - then restart Bedlam and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@bedlam.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "bedlam", path: "/api/skills/bedlam" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "bedlam-create-agent",
          path: "/api/skills/bedlam-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              bedlamApiUrl: req.body.bedlamApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.bedlamApiUrl &&
          !persistedDefaults.bedlamApiUrl
        ) {
          missingPersistedFields.push("bedlamApiUrl");
        }
