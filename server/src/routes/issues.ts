...
export function issueRoutes(db: Db, storage: StorageService) {
  ...
  router.post("/issues/:id/release", async (req, res) => {
    ...
    const clearExecution =
      ...
      req.body !== null &&
      (req.body as Record<string, unknown>).clearExecution === true;
    if (!clearExecution && !(await assertAgentRunCheckoutOwnership(req, res, existing))) return;
    ...
    const actor = getActorInfo(req);
    const releaseDetails = clearExecution
      ? existing.executionRunId
        ? {
          clearExecution: true,
          ...(existing.checkoutRunId && released.checkoutRunId !== existing.checkoutRunId
            ? { clearedCheckoutRunId: existing.checkoutRunId }
            : {}),
          clearedExecutionRunId: existing.executionRunId,
          clearedExecutionAgentNameKey: existing.executionAgentNameKey,
          clearedExecutionLockedAt: existing.executionLockedAt instanceof Date
            ? existing.executionLockedAt.toISOString()
