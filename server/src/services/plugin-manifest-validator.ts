 ...
 */
import { pluginManifestV1Schema } from "@bedlam/shared";
import type { BedlamPluginManifestV1 } from "@bedlam/shared";
import { PLUGIN_API_VERSION } from "@bedlam/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): BedlamPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as BedlamPluginManifestV1,
        };
      }
