...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@bedlam/db";
 ...
 *
 * This implementation delegates to the core Bedlam domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@bedlam/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
