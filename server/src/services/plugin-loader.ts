 ...
 *    (`~/.bedlam/plugins/`) and `node_modules` for packages matching
 *    the `bedlam-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Bedlam plugins.
 * Packages matching this pattern are considered Bedlam plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "bedlam-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: BedlamPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.bedlam/plugins/ local directory
  | "npm"               // npm packages matching bedlam-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: BedlamPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the bedlam-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "bedlam-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: BedlamPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Bedlam plugins installed as npm packages in the current
   * Node.js environment matching the "bedlam-plugin-*" naming convention.
   *
   ...
   * the "bedlamPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Bedlam plugin.
   * Throws if the package is a Bedlam plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<BedlamPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: BedlamPluginManifestV1;
    newManifest: BedlamPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Bedlam plugin naming convention.
 * Accepts both the "bedlam-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @bedlam/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "bedlamPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const bedlamPlugin = pkgJson["bedlamPlugin"];
  if (
    bedlamPlugin !== null &&
    typeof bedlamPlugin === "object" &&
    !Array.isArray(bedlamPlugin)
  ) {
    const manifestRelPath = (bedlamPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: BedlamPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumBedlamVersion;
}
 ...
 *   packageName: "bedlam-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: BedlamPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as BedlamPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Bedlam plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<BedlamPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Bedlam plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasBedlamPlugin = "bedlamPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasBedlamPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<BedlamPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: BedlamPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @bedlam/shared exports). Run those workers through
