...
import type { BedlamPluginManifestV1 } from "@bedlam/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __bedlam_exports: module.exports,
      __bedlam_module: module,
      __bedlam_require: requireInSandbox,
      __bedlam_filename: realPath,
      __bedlam_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__bedlam_exports, __bedlam_module, __bedlam_require, __bedlam_filename, __bedlam_dirname)`;
