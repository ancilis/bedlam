...
import { companySkills } from "@bedlam/db";
import { readBedlamSkillSyncPreference, writeBedlamSkillSyncPreference } from "@bedlam/adapter-utils/server-utils";
import type { BedlamSkillEntry } from "@bedlam/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@bedlam/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveBedlamInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("bedlam/bedlam/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveBedlamInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "bedlam_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Bedlam skills are read-only.",
      sourceLabel: "Bedlam bundled",
      sourceBadge: "bedlam",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "bedlam_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Bedlam skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "bedlam_bundled"
        && incomingKind === "github"
        && incomingOwner === "bedlam"
        && incomingRepo === "bedlam"
      ) {
        out.push(existing);
        continue;
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeBedlamSkillSyncPreference(config, filtered),
        });
      }
    }
