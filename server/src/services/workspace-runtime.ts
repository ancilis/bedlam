...
import type { AdapterRuntimeServiceReport } from "@bedlam/adapter-utils";
import type { Db } from "@bedlam/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@bedlam/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.BEDLAM_WORKSPACE_CWD = input.worktreePath;
  env.BEDLAM_WORKSPACE_PATH = input.worktreePath;
  env.BEDLAM_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.BEDLAM_WORKSPACE_BRANCH = input.branchName;
  env.BEDLAM_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.BEDLAM_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.BEDLAM_WORKSPACE_SOURCE = input.base.source;
  env.BEDLAM_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.BEDLAM_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.BEDLAM_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.BEDLAM_PROJECT_ID = input.base.projectId ?? "";
  env.BEDLAM_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.BEDLAM_AGENT_ID = input.agent.id ?? "";
  env.BEDLAM_AGENT_NAME = input.agent.name;
  env.BEDLAM_COMPANY_ID = input.agent.companyId;
  env.BEDLAM_ISSUE_ID = input.issue?.id ?? "";
  env.BEDLAM_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.BEDLAM_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.BEDLAM_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.BEDLAM_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.BEDLAM_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.BEDLAM_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.BEDLAM_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.BEDLAM_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.BEDLAM_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.BEDLAM_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.BEDLAM_PROJECT_ID = input.workspace.projectId ?? "";
  env.BEDLAM_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.BEDLAM_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".bedlam", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Bedlam", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
