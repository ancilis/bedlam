...
import { extractAgentMentionIds, extractProjectMentionIds } from "@bedlam/shared";
...
export function issueService(db: Db) {
  ...
  async function recoverStaleExecutionRunOnCheckout(input: {
    issueId: string;
    actorAgentId: string;
    actorRunId: string;
    ...
    const recovered = await db
      ...
      .where(
        and(
          ...
          or(isNull(issues.checkoutRunId), eq(issues.checkoutRunId, input.actorRunId)),
  ...
  return {
    ...
    checkout: async (id: string, agentId: string, expectedStatuses: string[], checkoutRunId: string | null) => {
      ...
      if (
        ...
        (current.checkoutRunId == null || current.checkoutRunId === checkoutRunId) &&
        expectedStatuses.includes(current.status)
      ) {
        const recovered = await recoverStaleExecutionRunOnCheckout({
          issueId: id,
    ...
    release: async (id: string, actorAgentId?: string, actorRunId?: string | null, options?: IssueReleaseOptions) => {
      const existing = await db
        .select()
        .from(issues)
        .where(eq(issues.id, id))
      ...
      if (options?.clearExecution) {
        if (!existing.executionRunId) {
          const [enriched] = await withIssueLabels(db, [existing]);
          return enriched;
        }

        const clearable = await isTerminalOrMissingHeartbeatRun(existing.executionRunId);
        if (!clearable) {
          throw conflict("Cannot clear active execution lock", {
            issueId: existing.id,
        ...
        const updated = await db
          .update(issues)
          .set({
            checkoutRunId: existing.checkoutRunId === existing.executionRunId ? null : existing.checkoutRunId,
            executionRunId: null,
            executionAgentNameKey: null,
          ...
          .where(
            and(
              eq(issues.id, id),
              eq(issues.executionRunId, existing.executionRunId),
              existing.checkoutRunId ? eq(issues.checkoutRunId, existing.checkoutRunId) : isNull(issues.checkoutRunId),
            ),
          )
          .returning()
          .then((rows) => rows[0] ?? null);
