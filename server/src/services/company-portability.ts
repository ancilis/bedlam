...
import {
  readBedlamSkillSyncPreference,
  writeBedlamSkillSyncPreference,
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".bedlam.yaml" || normalized === ".bedlam.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const bedlam = isPlainRecord(metadata?.bedlam) ? metadata?.bedlam as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.bedlamSkillKey)
    ?? asString(bedlam?.skillKey)
    ?? asString(bedlam?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "bedlam_bundled") {
    return `bedlam/bedlam/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .bedlam.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .bedlam.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .bedlam.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .bedlam.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Bedlam will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .bedlam.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .bedlam.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .bedlam.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .bedlam.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .bedlam.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .bedlam.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .bedlam.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .bedlam.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .bedlam.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[bedlamExtensionPath];
  if (selectedFiles.has(bedlamExtensionPath) && typeof extensionEntry === "string") {
    filtered[bedlamExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findBedlamExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".bedlam.yaml"] === "string") return ".bedlam.yaml";
  if (typeof files[".bedlam.yml"] === "string") return ".bedlam.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.bedlam.yaml") || entry.endsWith("/.bedlam.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "bedlam_bundled") {
    ...
    return {
      ...
      commit,
      trackingRef: "master",
      url: `https://github.com/bedlam/bedlam/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "bedlam_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.bedlamSkillKey = skill.key;
  metadata.bedlam = {
    ...(isPlainRecord(metadata.bedlam) ? metadata.bedlam : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const bedlamExtensionPath = findBedlamExtensionPath(normalizedFiles);
  const bedlamExtension = bedlamExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, bedlamExtensionPath) ?? "")
    : {};
  const bedlamCompany = isPlainRecord(bedlamExtension.company) ? bedlamExtension.company : {};
  const bedlamSidebar = normalizePortableSidebarOrder(bedlamExtension.sidebar);
  const bedlamAgents = isPlainRecord(bedlamExtension.agents) ? bedlamExtension.agents : {};
  const bedlamProjects = isPlainRecord(bedlamExtension.projects) ? bedlamExtension.projects : {};
  const bedlamTasks = isPlainRecord(bedlamExtension.tasks) ? bedlamExtension.tasks : {};
  const bedlamRoutines = isPlainRecord(bedlamExtension.routines) ? bedlamExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(bedlamCompany.brandColor),
      logoPath: asString(bedlamCompany.logoPath) ?? asString(bedlamCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof bedlamCompany.requireBoardApprovalForNewAgents === "boolean"
          ? bedlamCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
    },
    sidebar: bedlamSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(bedlamAgents[slug]) ? bedlamAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(bedlamProjects[slug]) ? bedlamProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(bedlamTasks[slug]) ? bedlamTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(bedlamRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(bedlamRoutines[slug]) ? bedlamRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const bedlamAgentsOut: Record<string, Record<string, unknown>> = {};
    const bedlamProjectsOut: Record<string, Record<string, unknown>> = {};
    const bedlamTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const bedlamRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readBedlamSkillSyncPreference(
        ...
        bedlamAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      bedlamProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      bedlamTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      bedlamRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const bedlamExtensionPath = ".bedlam.yaml";
    const bedlamAgents = Object.fromEntries(
      Object.entries(bedlamAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const bedlamProjects = Object.fromEntries(
      Object.entries(bedlamProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const bedlamTasks = Object.fromEntries(
      Object.entries(bedlamTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const bedlamRoutines = Object.fromEntries(
      Object.entries(bedlamRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[bedlamExtensionPath] = buildYamlFile(
      {
        schema: "bedlam/v1",
        ...
        agents: Object.keys(bedlamAgents).length > 0 ? bedlamAgents : undefined,
        projects: Object.keys(bedlamProjects).length > 0 ? bedlamProjects : undefined,
        tasks: Object.keys(bedlamTasks).length > 0 ? bedlamTasks : undefined,
        routines: Object.keys(bedlamRoutines).length > 0 ? bedlamRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, bedlamExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeBedlamSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
