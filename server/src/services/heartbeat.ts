...
import type { BillingType, ExecutionWorkspace, ExecutionWorkspaceConfig } from "@bedlam/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_bedlamWakeContext";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__bedlam_repo_only__";
...
// --- Claude → Codex usage-limit fallback ---------------------------------
// When claude_local hits a usage / rate limit error, automatically re-run the
// same heartbeat with codex_local. The codex_local adapter implements the same
// AdapterExecutionContext / AdapterExecutionResult contract, so we can swap
// adapters mid-run without disturbing the surrounding heartbeat plumbing.
//
// We deliberately:
//   * never fall back on auth-required errors (that's a credential problem,
//     not a quota problem — operator must re-login)
//   * never fall back on timeouts (timeouts are not quota issues)
//   * scrub claude-only config keys (model="claude-sonnet-4-6", etc.) before
//     handing the config to codex, since codex would otherwise pass an unknown
//     model id to `codex exec --model …`
//   * clear the session in the persisted result so the next claude_local run
//     starts fresh after the limit resets, instead of trying to resume what
//     codex returned

const CLAUDE_USAGE_LIMIT_RE =
  /(usage[\s_-]?limit|rate[\s_-]?limit(?:ed|_error|_exceeded)?|5[\s-]?hour limit|weekly limit|\b429\b|rate_limited|approaching usage|you'?ve reached your usage|claude ai usage limit)/i;

function adapterResultLooksLikeClaudeUsageLimit(
  result: AdapterExecutionResult | null | undefined,
): boolean {
  if (!result) return false;
  if (result.timedOut) return false;
  if (result.errorCode === "claude_auth_required") return false;
  if ((result.exitCode ?? 0) === 0 && !result.errorMessage) return false;

  const candidates: string[] = [];
  if (typeof result.errorMessage === "string") candidates.push(result.errorMessage);
  const json = result.resultJson as Record<string, unknown> | null | undefined;
  if (json && typeof json === "object") {
    const subtype = json.subtype;
    if (typeof subtype === "string") candidates.push(subtype);
    const resultText = json.result;
    if (typeof resultText === "string") candidates.push(resultText);
    const stdout = (json as { stdout?: unknown }).stdout;
    const stderr = (json as { stderr?: unknown }).stderr;
    if (typeof stdout === "string") candidates.push(stdout);
    ...
    if (Array.isArray(errors)) {
      for (const err of errors) {
        if (typeof err === "string") {
          ...
          if (typeof errType === "string") candidates.push(errType);
        }
      }
    }
  }
  return candidates.some((text) => CLAUDE_USAGE_LIMIT_RE.test(text));
}

function buildCodexFallbackConfig(
  originalConfig: Record<string, unknown>,
): Record<string, unknown> {
  const fallback: Record<string, unknown> = { ...originalConfig };
  // Force codex to use its CLI default model (~/.codex/config.toml). Claude
  // model identifiers like "claude-sonnet-4-6" would crash `codex exec -m …`.
  fallback.model = "";
  // Force codex command, in case the agent's claude config explicitly sets
  // command: "claude". Most agents do not, but be defensive.
  fallback.command = "codex";
  // Mirror claude's `dangerouslySkipPermissions: true` (which Bedlam uses
  // for unattended runs) onto codex's equivalent, so the fallback inherits
  // the same trust posture instead of stalling on approval prompts.
  if (originalConfig.dangerouslySkipPermissions === true) {
    fallback.dangerouslyBypassApprovalsAndSandbox = true;
  }
  // Drop claude-only keys so codex never reads stale values out of them.
  delete fallback.dangerouslySkipPermissions;
  delete fallback.maxTurnsPerRun;
  return fallback;
}
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    const runtimeConfig = {
      ...resolvedConfig,
      bedlamRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.bedlamWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.bedlamRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.bedlamRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.bedlamSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.bedlamSessionRotationReason = sessionCompaction.reason;
      context.bedlamPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.bedlamSessionHandoffMarkdown;
      delete context.bedlamSessionRotationReason;
      delete context.bedlamPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.bedlamRuntimeServices = runtimeServices;
        context.bedlamRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[bedlam] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected BEDLAM_API_KEY",
        );
      }
      let adapterResult: AdapterExecutionResult = await adapter.execute({
        runId: run.id,
        agent,
      ...
      // --- Claude → Codex fallback on usage / rate limit -----------------
      // If the agent is configured for claude_local and the run failed with
      // what looks like an Anthropic usage / rate-limit error, transparently
      // re-run the SAME heartbeat with codex_local. This keeps Bedlam
      // making progress when Claude credits are exhausted.
      if (
        agent.adapterType === "claude_local" &&
        adapterResultLooksLikeClaudeUsageLimit(adapterResult)
      ) {
        const fallbackAdapterType = "codex_local";
        const fallbackAdapter = getServerAdapter(fallbackAdapterType);
        const fallbackConfig = buildCodexFallbackConfig(runtimeConfig);
        const fallbackAuthToken = fallbackAdapter.supportsLocalAgentJwt
          ...
          : null;
        const claudeErrorSummary =
          (adapterResult.errorMessage ?? "claude usage limit").slice(0, 240);

        await appendRunEvent(currentRun, seq++, {
          eventType: "adapter.fallback",
          stream: "system",
          level: "warn",
          message: "claude_local hit usage/rate limit; falling back to codex_local",
          payload: {
            from: "claude_local",
            to: fallbackAdapterType,
            reason: claudeErrorSummary,
            originalErrorCode: adapterResult.errorCode ?? null,
            originalExitCode: adapterResult.exitCode ?? null,
          },
        });
        await onLog(
          "stderr",
          `[bedlam] claude_local usage limit detected (${claudeErrorSummary}). Falling back to ${fallbackAdapterType}.\n`,
        );
        ...
        logger.warn(
          {
            ...
            errorMessage: claudeErrorSummary,
          },
          "claude_local usage limit detected; falling back to codex_local",
        );

        const codexResult = await fallbackAdapter.execute({
          runId: run.id,
          agent,
          runtime: runtimeForAdapter,
        ...
        // Don't persist codex's session id under the agent's claude_local
        // session slot — the next claude run wouldn't be able to resume it.
        // Force a clean session so claude starts fresh after limits reset.
        adapterResult = {
          ...codexResult,
          sessionId: null,
          sessionParams: null,
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.bedlamRuntimeServices = combinedRuntimeServices;
        context.bedlamRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[bedlam] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
