const FAVICON_BLOCK_START = "<!-- BEDLAM_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- BEDLAM_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- BEDLAM_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- BEDLAM_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.BEDLAM_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.BEDLAM_WORKTREE_NAME) ?? nonEmpty(env.BEDLAM_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.BEDLAM_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="bedlam-worktree-enabled" content="true" />',
    `<meta name="bedlam-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="bedlam-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="bedlam-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
