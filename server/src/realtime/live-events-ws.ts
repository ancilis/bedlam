...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@bedlam/db";
import type { DeploymentMode } from "@bedlam/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  bedlamUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).bedlamUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.bedlamUpgradeContext = context;
