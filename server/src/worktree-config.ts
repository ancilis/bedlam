...
import type { BedlamConfig } from "@bedlam/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Bedlam worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.BEDLAM_IN_WORKTREE !== "true") return null;

  const configPath = resolveBedlamConfigPath(overrideConfigPath);
  const envPath = resolveBedlamEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.BEDLAM_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.BEDLAM_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.BEDLAM_HOME) ??
      nonEmpty(env.BEDLAM_WORKTREES_DIR) ??
      "~/.bedlam-worktrees",
  );
...
function writeConfigFile(configPath: string, config: BedlamConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".bedlam", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as BedlamConfig;
...
): { config: BedlamConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.BEDLAM_HOME = context.homeDir;
  process.env.BEDLAM_INSTANCE_ID = context.instanceId;
  process.env.BEDLAM_CONFIG = context.configPath;
  process.env.BEDLAM_CONTEXT = context.contextPath;
  process.env.BEDLAM_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as BedlamConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    BEDLAM_HOME: context.homeDir,
    BEDLAM_INSTANCE_ID: context.instanceId,
    BEDLAM_CONFIG: context.configPath,
    BEDLAM_CONTEXT: context.contextPath,
    BEDLAM_IN_WORKTREE: "true",
    BEDLAM_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: BedlamConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as BedlamConfig;
  } catch {
    return;
  }
