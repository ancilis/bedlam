...
import { ensureCursorSkillsInjected } from "@bedlam/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Bedlam skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("bedlam-cursor-skills-src-");
    const skillsHome = await makeTempDir("bedlam-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "bedlam");
    await createSkillDir(skillsDir, "bedlam-create-agent");
    ...
    const injectedA = path.join(skillsHome, "bedlam");
    const injectedB = path.join(skillsHome, "bedlam-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "bedlam")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "bedlam-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "bedlam"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "bedlam-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("bedlam-cursor-preserve-src-");
    const skillsHome = await makeTempDir("bedlam-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "bedlam");
    ...
    expect((await fs.lstat(path.join(skillsHome, "bedlam-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("bedlam-cursor-fail-src-");
    const skillsHome = await makeTempDir("bedlam-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
