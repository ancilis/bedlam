...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const bedlamDir = path.join(worktreeRoot, ".bedlam");
    const configPath = path.join(bedlamDir, "config.json");
    const envPath = path.join(bedlamDir, ".env");
    const sharedRoot = path.join(tempRoot, ".bedlam", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".bedlam-worktrees");

    await fs.mkdir(bedlamDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Bedlam environment variables",
        "BEDLAM_IN_WORKTREE=true",
        "BEDLAM_WORKTREE_NAME=PAP-884-ai-commits-component",
        "BEDLAM_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.BEDLAM_IN_WORKTREE = "true";
    process.env.BEDLAM_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.BEDLAM_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`BEDLAM_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('BEDLAM_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`BEDLAM_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`BEDLAM_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('BEDLAM_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.BEDLAM_HOME).toBe(isolatedHome);
    expect(process.env.BEDLAM_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Bedlam environment variables",
        "BEDLAM_IN_WORKTREE=true",
        "BEDLAM_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.BEDLAM_IN_WORKTREE = "true";
    process.env.BEDLAM_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.BEDLAM_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".bedlam-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".bedlam", "worktrees");
    ...
    const bedlamDir = path.join(currentWorktreeRoot, ".bedlam");
    const configPath = path.join(bedlamDir, "config.json");
    const envPath = path.join(bedlamDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".bedlam", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.BEDLAM_IN_WORKTREE = "true";
    process.env.BEDLAM_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.BEDLAM_HOME = isolatedHome;
    process.env.BEDLAM_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.BEDLAM_CONFIG = configPath;
