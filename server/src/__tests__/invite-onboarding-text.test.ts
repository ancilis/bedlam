...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Bedlam OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Bedlam base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("bedlamApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.bedlamApiUrl");
    expect(text).toContain("~/.openclaw/workspace/bedlam-claimed-api-key.json");
    expect(text).toContain("BEDLAM_API_KEY");
