...
import type { CompanyPortabilityFileEntry } from "@bedlam/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "bedlam",
        name: "bedlam",
        description: "Bedlam coordination skill",
        markdown: "---\nname: bedlam\ndescription: Bedlam coordination skill\n---\n\n# Bedlam\n",
        sourceType: "github",
        sourceLocator: "https://github.com/bedlam/bedlam/tree/master/skills/bedlam",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/bedlam/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "bedlam",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/bedlam/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "bedlam",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Bedlam extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Bedlam"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${bedlamKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/bedlam/bedlam/bedlam/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/bedlam/bedlam/bedlam/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/bedlam/bedlam/bedlam/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".bedlam.yaml"]);
    expect(extension).toContain('schema: "bedlam/v1"');
    ...
    expect(extension).not.toContain("bedlamSkillSync");
  ...
  it("exports default sidebar order into the Bedlam extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".bedlam.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/bedlam/bedlam/bedlam/SKILL.md"])).toContain("# Bedlam");
    expect(asTextFile(exported.files["skills/bedlam/bedlam/bedlam/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/bedlam/bedlam/bedlam/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/bedlam/bedlam/bedlam/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/bedlam/bedlam/bedlam/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .bedlam.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".bedlam.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-bedlam",
        companyId: "company-1",
        key: "bedlam/bedlam/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/bedlam/bedlam/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "bedlam_bundled",
          owner: "bedlam",
          repo: "bedlam",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/bedlam/bedlam/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/bedlam/bedlam/release-changelog/SKILL.md"])).toContain("bedlam/bedlam/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/bedlam",
            repoUrl: "https://github.com/bedlam/bedlam.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .bedlam-tmp",
            remoteProvider: null,
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Bedlam",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/bedlam/bedlam.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".bedlam.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/bedlam/bedlam.git"');
    expect(extension).toContain('projectWorkspaceKey: "bedlam"');
  ...
  it("reads env inputs back from .bedlam.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Bedlam routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "bedlam-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Bedlam" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Bedlam"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .bedlam.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Bedlam");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".bedlam.yaml"] = `${exported.files[".bedlam.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
