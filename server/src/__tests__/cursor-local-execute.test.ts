...
import { execute } from "@bedlam/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects bedlam env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the bedlam heartbeat.");
      ...
      expect(capture.prompt).toContain("Bedlam runtime note:");
      expect(capture.prompt).toContain("BEDLAM_API_KEY");
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const bedlamDir = await createSkillDir(runtimeSkillsRoot, "bedlam");
