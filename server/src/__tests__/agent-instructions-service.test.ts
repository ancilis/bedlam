...
describe("agent instructions service", () => {
  const originalBedlamHome = process.env.BEDLAM_HOME;
  const originalBedlamInstanceId = process.env.BEDLAM_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalBedlamHome === undefined) delete process.env.BEDLAM_HOME;
    else process.env.BEDLAM_HOME = originalBedlamHome;
    if (originalBedlamInstanceId === undefined) delete process.env.BEDLAM_INSTANCE_ID;
    else process.env.BEDLAM_INSTANCE_ID = originalBedlamInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const bedlamHome = await makeTempDir("bedlam-agent-instructions-home-");
    const externalRoot = await makeTempDir("bedlam-agent-instructions-external-");
    cleanupDirs.add(bedlamHome);
    cleanupDirs.add(externalRoot);
    process.env.BEDLAM_HOME = bedlamHome;
    process.env.BEDLAM_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("bedlam-agent-instructions-new-external-");
    cleanupDirs.add(bedlamHome);
    cleanupDirs.add(externalRoot);
    process.env.BEDLAM_HOME = bedlamHome;
    process.env.BEDLAM_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("bedlam-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const bedlamHome = await makeTempDir("bedlam-agent-instructions-recover-");
    cleanupDirs.add(bedlamHome);
    process.env.BEDLAM_HOME = bedlamHome;
    process.env.BEDLAM_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const bedlamHome = await makeTempDir("bedlam-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("bedlam-agent-instructions-stale-root-");
    cleanupDirs.add(bedlamHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const bedlamHome = await makeTempDir("bedlam-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("bedlam-agent-instructions-heal-write-stale-");
    cleanupDirs.add(bedlamHome);
    cleanupDirs.add(staleRoot);
    process.env.BEDLAM_HOME = bedlamHome;
    process.env.BEDLAM_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const bedlamHome = await makeTempDir("bedlam-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("bedlam-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(bedlamHome);
    cleanupDirs.add(staleRoot);
    process.env.BEDLAM_HOME = bedlamHome;
    process.env.BEDLAM_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const bedlamHome = await makeTempDir("bedlam-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("bedlam-agent-instructions-partial-root-");
    cleanupDirs.add(bedlamHome);
    cleanupDirs.add(staleRoot);
    process.env.BEDLAM_HOME = bedlamHome;
    process.env.BEDLAM_INSTANCE_ID = "test-instance";
