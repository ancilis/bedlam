...
import { resolveBedlamConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@bedlam/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "bedlam@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Bedlam Test"]);
...
afterEach(async () => {
  ...
  delete process.env.BEDLAM_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Bedlam and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/bedlam",
      BEDLAM_HOME: "/tmp/bedlam-home",
      BEDLAM_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.BEDLAM_HOME).toBeUndefined();
    expect(sanitized.BEDLAM_INSTANCE_ID).toBeUndefined();
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".bedlam", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$BEDLAM_WORKSPACE_BRANCH\" > .bedlam-provision-branch",
        "printf '%s\\n' \"$BEDLAM_WORKSPACE_BASE_CWD\" > .bedlam-provision-base",
        "printf '%s\\n' \"$BEDLAM_WORKSPACE_CREATED\" > .bedlam-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".bedlam-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".bedlam-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".bedlam-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".bedlam-provision-created"), "utf8")).resolves.toBe("false\n");
  });

  it("writes an isolated repo-local Bedlam config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const bedlamHome = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(bedlamHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.BEDLAM_HOME = bedlamHome;
    process.env.BEDLAM_INSTANCE_ID = instanceId;
    process.env.BEDLAM_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/bedlam"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".bedlam", "config.json");
      const envPath = path.join(workspace.cwd, ".bedlam", ".env");
      ...
      expect(envContents).toContain(`BEDLAM_HOME=${JSON.stringify(isolatedWorktreeHome)}`);
      expect(envContents).toContain(`BEDLAM_INSTANCE_ID=${JSON.stringify(expectedInstanceId)}`);
      expect(envContents).toContain(`BEDLAM_CONFIG=${JSON.stringify(configPath)}`);
      expect(envContents).toContain("BEDLAM_IN_WORKTREE=true");
      expect(envContents).toContain(
        `BEDLAM_WORKTREE_NAME=${JSON.stringify("PAP-885-show-worktree-banner")}`,
      );

      process.chdir(workspace.cwd);
      expect(resolveBedlamConfigPath()).toBe(configPath);
    } finally {
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-runtime-workspace-"));
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".bedlam", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.BEDLAM_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              BEDLAM_HOME: "{{workspace.cwd}}/.bedlam/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".bedlam", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".bedlam", "runtime-services"));
  });

  it("does not leak parent Bedlam instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "bedlamConfig: process.env.BEDLAM_CONFIG ?? null,",
          "bedlamHome: process.env.BEDLAM_HOME ?? null,",
          "bedlamInstanceId: process.env.BEDLAM_INSTANCE_ID ?? null,",
    ...
    process.env.BEDLAM_CONFIG = "/tmp/base-bedlam-config.json";
    process.env.BEDLAM_HOME = "/tmp/base-bedlam-home";
    process.env.BEDLAM_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/bedlam";
    ...
    expect(captured.bedlamConfig).toBeNull();
    expect(captured.bedlamHome).toBeNull();
    expect(captured.bedlamInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-runtime-sibling-"));
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("bedlam-workspace-runtime-");
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-runtime-reconcile-"));
    const bedlamHome = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-runtime-home-"));
    process.env.BEDLAM_HOME = bedlamHome;
    process.env.BEDLAM_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bedlam-runtime-stop-persisted-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
