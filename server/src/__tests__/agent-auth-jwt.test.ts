...
describe("agent local JWT", () => {
  const secretEnv = "BEDLAM_AGENT_JWT_SECRET";
  const ttlEnv = "BEDLAM_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "BEDLAM_AGENT_JWT_ISSUER";
  const audienceEnv = "BEDLAM_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "bedlam";
    process.env[audienceEnv] = "bedlam-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
