...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "bedlam", LOGNAME: "bedlam", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "bedlam" }),
      },
    );

    expect(tokens).toEqual(["bedlam", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "bedlam" }, {
        userInfo: () => ({ username: "bedlam" }),
      });

      expect(tokens).toEqual(["bedlam", "custom-token"]);
    } finally {
