...
function jwtConfig() {
  const secret = process.env.BEDLAM_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.BEDLAM_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.BEDLAM_AGENT_JWT_ISSUER ?? "bedlam",
    audience: process.env.BEDLAM_AGENT_JWT_AUDIENCE ?? "bedlam-api",
  };
}
