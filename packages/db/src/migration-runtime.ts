...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://bedlam:bedlam@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "bedlam");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://bedlam:bedlam@127.0.0.1:${preferredPort}/bedlam`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://bedlam:bedlam@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "bedlam");
    return {
      connectionString: `postgres://bedlam:bedlam@127.0.0.1:${port}/bedlam`,
  ...
  const adminConnectionString = `postgres://bedlam:bedlam@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "bedlam");

  return {
    connectionString: `postgres://bedlam:bedlam@127.0.0.1:${selectedPort}/bedlam`,
