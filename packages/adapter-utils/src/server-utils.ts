...
const BEDLAM_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface BedlamSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: BedlamSkillEntry[];
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "BEDLAM_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildBedlamEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.BEDLAM_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.BEDLAM_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.BEDLAM_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.BEDLAM_API_URL = apiUrl;
  return vars;
}
...
export async function resolveBedlamSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...BEDLAM_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listBedlamSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<BedlamSkillEntry[]> {
  const root = await resolveBedlamSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Bedlam skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Bedlam cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredBedlamRuntimeSkills(value: unknown): BedlamSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: BedlamSkillEntry[] = [];
...
export async function readBedlamRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<BedlamSkillEntry[]> {
  const configuredEntries = normalizeConfiguredBedlamRuntimeSkills(config.bedlamRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listBedlamSkillEntries(moduleDir, additionalCandidates);
}

export async function readBedlamSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listBedlamSkillEntries(moduleDir);
...
export function readBedlamSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.bedlamSkillSync;
...
function canonicalizeDesiredBedlamSkillReference(
  reference: string,
...
export function resolveBedlamDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readBedlamSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredBedlamSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeBedlamSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.bedlamSkillSync;
  const current =
  ...
  next.bedlamSkillSync = current;
  return next;
}

export async function ensureBedlamSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Bedlam server itself is started from
    // within a Claude Code session (e.g. `npx bedlam run` in a terminal
