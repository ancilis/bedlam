...
import { asString, parseObject } from "@bedlam/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Bedlam server host.",
        });
      }
    } catch (err) {
      checks.push({
