...
import { asNumber, asString, buildBedlamEnv, parseObject } from "@bedlam/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "bedlam";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "bedlam";
  if (input.strategy === "run") return `bedlam:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `bedlam:issue:${input.issueId}`;
  return fallback;
}
...
function resolveBedlamApiUrlOverride(value: unknown): string | null {
...
function buildBedlamEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const bedlamApiUrlOverride = resolveBedlamApiUrlOverride(ctx.config.bedlamApiUrl);
  const bedlamEnv: Record<string, string> = {
    ...buildBedlamEnv(ctx.agent),
    BEDLAM_RUN_ID: ctx.runId,
  };

  if (bedlamApiUrlOverride) {
    bedlamEnv.BEDLAM_API_URL = bedlamApiUrlOverride;
  }
  if (wakePayload.taskId) bedlamEnv.BEDLAM_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) bedlamEnv.BEDLAM_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) bedlamEnv.BEDLAM_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) bedlamEnv.BEDLAM_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) bedlamEnv.BEDLAM_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    bedlamEnv.BEDLAM_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return bedlamEnv;
}

function buildWakeText(payload: WakePayload, bedlamEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/bedlam-claimed-api-key.json";
  const orderedKeys = [
    "BEDLAM_RUN_ID",
    "BEDLAM_AGENT_ID",
    "BEDLAM_COMPANY_ID",
    ...
    "BEDLAM_TASK_ID",
    "BEDLAM_WAKE_REASON",
    "BEDLAM_WAKE_COMMENT_ID",
    "BEDLAM_APPROVAL_ID",
    "BEDLAM_APPROVAL_STATUS",
    "BEDLAM_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = bedlamEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = bedlamEnv.BEDLAM_API_URL ?? "<set BEDLAM_API_URL>";

  const lines = [
    "Bedlam wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `BEDLAM_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load BEDLAM_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $BEDLAM_API_KEY on every API call.",
    "- Use X-Bedlam-Run-Id: $BEDLAM_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: BEDLAM_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$BEDLAM_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$BEDLAM_COMPANY_ID/issues?assigneeAgentId=$BEDLAM_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardBedlamPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  bedlamEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateBedlam = parseObject(payloadTemplate.bedlam);
  const workspace = asRecord(ctx.context.bedlamWorkspace);
  const workspaces = Array.isArray(ctx.context.bedlamWorkspaces)
    ? ctx.context.bedlamWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.bedlamRuntimeServiceIntents)
    ? ctx.context.bedlamRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardBedlam: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: bedlamEnv.BEDLAM_API_URL ?? null,
  };

  if (workspace) {
    standardBedlam.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardBedlam.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardBedlam.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "bedlam-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const bedlamEnv = buildBedlamEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, bedlamEnv);
  ...
  const bedlamPayload = buildStandardBedlamPayload(ctx, wakePayload, bedlamEnv, payloadTemplate);
