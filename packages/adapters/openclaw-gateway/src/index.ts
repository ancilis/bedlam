...
Use when:
- You want Bedlam to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Bedlam server.
...
- bedlamApiUrl (string, optional): absolute Bedlam base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default bedlam)

Standard outbound payload additions:
- bedlam (object): standardized Bedlam context added to every gateway agent request
- bedlam.workspace (object, optional): resolved execution workspace for this run
- bedlam.workspaces (array, optional): additional workspace hints Bedlam exposed to the run
- bedlam.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
