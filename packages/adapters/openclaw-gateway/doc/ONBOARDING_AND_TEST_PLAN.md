...
## Scope
This plan is now **gateway-only**. Bedlam supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Bedlam skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Bedlam task.
...
```bash
pnpm --filter @bedlam/server test:run -- openclaw-gateway
pnpm --filter @bedlam/server typecheck
pnpm --filter @bedlam/ui typecheck
pnpm --filter bedlam typecheck
```
