...
Use when:
- You want Bedlam to run Cursor Agent CLI locally as the agent runtime
...
- Bedlam auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$bedlam" and related skills on local runs.
- Bedlam auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
