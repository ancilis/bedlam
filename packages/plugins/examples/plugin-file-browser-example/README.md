...
Example Bedlam plugin that demonstrates:
...
```bash
pnpm --filter @bedlam/plugin-file-browser-example build
pnpm bedlam plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm bedlam plugin uninstall bedlam-file-browser-example --force
```
...
- Optional: use `bedlam-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
