...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".bedlam-sdk");
...
/**
 * Generate a complete Bedlam plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Bedlam plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "bedlam-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["bedlam", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@bedlam/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@bedlam/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@bedlam/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@bedlam/plugin-sdk\` and \`@bedlam/shared\` from a local Bedlam checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.bedlam-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-bedlam-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <bedlam-sdk-path>]");
    process.exit(1);
  }
