...
Official TypeScript SDK for Bedlam plugin authors.

- **Worker SDK:** `@bedlam/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@bedlam/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@bedlam/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@bedlam/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@bedlam/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@bedlam/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@bedlam/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@bedlam/plugin-sdk/ui/hooks` | Hooks only |
| `@bedlam/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@bedlam/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@bedlam/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@bedlam/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@bedlam/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@bedlam/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Bedlam app. They can call ordinary Bedlam HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Bedlam instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@bedlam/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@bedlam/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Bedlam information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@bedlam/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@bedlam/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@bedlam/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@bedlam/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@bedlam/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@bedlam/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@bedlam/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@bedlam/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@bedlam/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@bedlam/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@bedlam/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@bedlam/plugin-sdk/bundlers";
...
```bash
bedlam-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@bedlam/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__bedlam__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__bedlam__/events` streams `reload` SSE events on UI build changes
