/**
 * Core types for the Bedlam plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@bedlam/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @bedlam/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Bedlam object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Bedlam UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Bedlam domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Bedlam domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Bedlam secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Bedlam secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@bedlam/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@bedlam/plugin-sdk";
 *
