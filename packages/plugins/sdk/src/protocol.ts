 ...
 * channel.
 *
 * The Bedlam plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@bedlam/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Bedlam messages.
 */
...
/**
 * Bedlam plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Bedlam instance. */
    instanceId: string;
    /** Semver version of the running Bedlam host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
